...
*/
 ...
 // SymbolTable Management ==============================================
    ...
    private void initSymbolTable()
    {
    	...
    	functionType = new FuncType(new TypeList(), new IntType());
    	symbolTable.getSymbol("readInt").setType(functionType);
    	
    	// readFloat() : float, Prompts the user for an integer.
    	this.symbolTable.insert("readFloat");
    	functionType = new FuncType(new TypeList(), new FloatType());
    	symbolTable.getSymbol("readFloat").setType(functionType);
    	
    	//printBool(arg:bool) : void, Prints a bool value to the screen.
    	this.symbolTable.insert("printBool");
    	arguments.append(new BoolType());
    	functionType = new FuncType( arguments, new VoidType());
    	symbolTable.getSymbol("printBool").setType(functionType);
    	...
    	functionType = new FuncType(arguments, new VoidType());
    	symbolTable.getSymbol("printInt").setType(functionType);
    	...
    	functionType = new FuncType(new TypeList(), new VoidType());
    	symbolTable.getSymbol("printFloat").setType(functionType);
    	
    	// println() : void, Prints newline character to the screen.
    	this.symbolTable.insert("println");
    	functionType = new FuncType(new TypeList(), new IntType());
    	symbolTable.getSymbol("println").setType(functionType);
    }
...
// Helper Methods ==========================================
    ...
    private TypeList getTypeList(List<Symbol> args)
    {
    	TypeList returnTypeList = new TypeList();
    	
    	for (Symbol symbol : args){
    		returnTypeList.add(symbol.type());
    	}
...
// Grammar Rules =====================================================
    ...
    public Type type() throws IOException
    {
    	...
    	Type returnType = tryResolveType(tok.lexeme());
    ...
    public ast.VariableDeclaration variable_declaration() throws IOException
    {
    	...
    	Token identifier = expectRetrieve(Token.Kind.IDENTIFIER);
    	Symbol variableSymbol = tryDeclareSymbol(identifier);
    	...
    	ast.VariableDeclaration variable = new ast.VariableDeclaration(varToken.lineNumber(), varToken.charPosition(), variableSymbol);
    	return variable;
    }
    ...
    public ast.ArrayDeclaration array_declaration() throws IOException
    {
    	...
    	Token arrayIdentifier = expectRetrieve(Token.Kind.IDENTIFIER);
    	Symbol arraySymbol = tryDeclareSymbol(arrayIdentifier);
    	...
    	ast.ArrayDeclaration arrayDeclaration = new ast.ArrayDeclaration(arrayToken.lineNumber(), arrayToken.charPosition(), arraySymbol);
    ...
    public ast.FunctionDefinition function_definition() throws IOException
    {
    	...
    	Token identifier = expectRetrieve(Token.Kind.IDENTIFIER);
    	Symbol funcSymbol = tryDeclareSymbol(identifier);
    	
    	expect(Token.Kind.OPEN_PAREN);
    	enterScope();
    	
    	List<Symbol> argsSymbols = parameter_list();
    	...
    	TypeList argsTypes = getTypeList(argsSymbols);
    	funcSymbol.setType(new FuncType(argsTypes, returnType));
    	...
    	return new ast.FunctionDefinition(funcToken.lineNumber(), funcToken.charPosition(), funcSymbol, argsSymbols, statementBlock);
    }
